#ifndef __DB_DATABASE_MY_SQL_H__
#define __DB_DATABASE_MY_SQL_H__
//---------------------------------------------------------------------------
//
...
// Description:
//
// This class handles calls to the MySQL native C API. This class links the
...
struct DBmySqlDllInfo
{
   COLstring Path;
   COLstring Version;
};

// DBfindSupportedMySqlDlls() searches for installed versions of MySQL on
...
//
COLarray<DBmySqlDllInfo> DBfindSupportedMySqlDlls(bool IgnoreCache = false);
...
{
public:
   DBdatabaseMySql(DBdatabaseType Type);
   ~DBdatabaseMySql();

   // initialize() attempts to load the MySQL shared library.  If a MySQL library has
   ...
   // reinitialize() will attempt to load a new MySQL shared library, replacing the one
   // loaded by initialize() if applicable.  It must be used to load a new MySQL shared
   ...
   //
   // reinitialize() cannot be used to unload all MySQL shared libraries, and will always
   // leave DBdatabaseMySql in a sane state, even if an exception is thrown.  Also,
   // it is safe to reinitialize() to the same library that is already loaded.
   //
   // *** We no longer support loading multiple MySQL libraries (#10153).  reinitialize()
   // *** will fail unless no MySQL library is loaded.  If readyToReinitialize() is false,
   ...
   // IMPLEMENTATION ONLY
   // This is used to initialize MySQL's thread local storage
...
private:
   ...
   DBdatabaseMySqlPrivate* pMember;
   DBmySqlDll& MySqlDll;
   bool m_BinaryOkay;



   DBdatabaseMySql( DBdatabaseMySql& Orig ); // not allowed
   DBdatabaseMySql& operator=( DBdatabaseMySql& Orig ); // not allowed
};
