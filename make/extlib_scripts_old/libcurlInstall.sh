LIB_NAME="libcurl"
LIB_REPO="https://github.com/curl/curl.git"
LIB_VERSION="curl-7_86_0"
...
./configure --enable-shared=no --disable-manual --disable-ldap --without-brotli --without-zstd --without-nghttp2 --prefix="$LIB_INSTALL_DIR" --with-openssl="$INSTALL_DIR/OpenSSL" --with-libssh2="$INSTALL_DIR/libssh2" --with-zlib="$INSTALL_DIR/zlib" >> $LIB_INSTALL_LOG 2>&1
if [ $? -ne 0 ]; then
