LIB_NAME="Git"
LIB_REPO="https://github.com/git/git.git"
LIB_VERSION="v2.38.1"
...
echo Configuring...
make configure >> $LIB_INSTALL_LOG 2>&1
if [[ "$OSTYPE" == "linux-gnu"* ]]; then
./configure --prefix="$LIB_INSTALL_DIR" --with-openssl="$INSTALL_DIR/OpenSSL" LDFLAGS="-L /usr/lib64/ -ldl" >> $LIB_INSTALL_LOG 2>&1
else
./configure --prefix="$LIB_INSTALL_DIR" --with-openssl="$INSTALL_DIR/OpenSSL" >> $LIB_INSTALL_LOG 2>&1
fi
if [ $? -ne 0 ]; then
...
echo Building...
NO_TCLTK=YesPlease RUNTIME_PREFIX=1 make >> $LIB_INSTALL_LOG 2>&1
if [ $? -ne 0 ]; then
...
echo Installing...
make INSTALL_SYMLINKS=1 install >> $LIB_INSTALL_LOG 2>&1
if [ $? -ne 0 ]; then
