#ifndef __IWEB_SERVER_SETTINGS_H__
#define __IWEB_SERVER_SETTINGS_H__
//---------------------------------------------------------------------------
// Copyright (C) 1997-2022 iNTERFACEWARE Inc.  All Rights Reserved
//
// Module: IWEBserverSettings
//
// Description:
//
// Web server settings page APIs: save settings, input validations
//
// Author: Shuo Hou
// Date:   Fri 19 Feb 2021 4:01:05 EDT
//---------------------------------------------------------------------------
class COLwebRequest;
class COLvar;
class IWEBconfig;
class SCKloop;

void IWEBpopulateServerInfo(COLvar* pResult, IWEBconfig* pWebConfig); // exposed for unit test
void IWEBserverSettings(const COLwebRequest& Request, IWEBconfig* pWebConfig, SCKloop* pLoop);

#endif // end of defensive include
