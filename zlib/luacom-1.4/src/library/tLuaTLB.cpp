...
int tLuaTLB::GetDocumentation(tLuaObject* lua_obj, lua_State* L)
{
  ...
  lua_pushstring(L, tUtil::bstr2string(name).c_str());
  lua_settable(L, -3);

  lua_pushstring(L, "helpstring");
  lua_pushstring(L, tUtil::bstr2string(helpstring).c_str());
  lua_settable(L, -3);

  lua_pushstring(L, "helpfile");
  lua_pushstring(L, tUtil::bstr2string(helpfile).c_str());
  lua_settable(L, -3);
...
int tLuaTLB::ShowHelp(tLuaObject* lua_obj, lua_State* L)
{
  ...
  tUtil::ShowHelp(tUtil::bstr2string(helpfile).c_str(), helpcontext);
...
int tLuaTypeInfo::GetFuncDesc(tLuaObject* lua_obj, lua_State* L)
{
  ...
  lua_pushstring(L, "name");
  lua_pushstring(L, tUtil::bstr2string(names[0]).c_str());
  lua_settable(L, -3);
  ...
  // gets other documentation
  {
    ...
    lua_pushstring(L, tUtil::bstr2string(description).c_str());
    lua_settable(L, -3);
  ...
  while(i < pfuncdesc->cParams)
  {
    ...
    if((i+1) < (SHORT)found)
    {
      lua_pushstring(L, tUtil::bstr2string(names[i+1]).c_str());
    ...
    if(!incompatible_type)
    {
      ...
      lua_pushstring(L, tCOMUtil::getPrintableTypeDesc(tdesc).c_str());
      lua_settable(L, -3);
    }
