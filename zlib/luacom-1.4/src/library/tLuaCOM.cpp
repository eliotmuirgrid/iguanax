...
// RCS Info
static char *rcsid = "$Id: tLuaCOM.cpp,v 1.2 2012-09-10 19:19:27 nasron Exp $";
                 ...
                 )
{
  ...
  //keep the increment/assignment atomic
   ...
   else // Houve erro
   {
     ...
     if(hr == DISP_E_EXCEPTION) // excecoes
     {
       if(excepinfo.bstrDescription != NULL)
         COM_EXCEPTION(tUtil::bstr2string(excepinfo.bstrDescription).c_str());
       else if(excepinfo.wCode != 0)
         COM_EXCEPTION(tUtil::GetErrorMessage(excepinfo.wCode).c_str());
       else if(excepinfo.scode != 0)
         COM_EXCEPTION(tUtil::GetErrorMessage(excepinfo.scode).c_str());
       else
         COM_EXCEPTION("Unknown exception");
     }
     else // Erros comuns
       COM_ERROR(tUtil::GetErrorMessage(hr).c_str());
   }
}
