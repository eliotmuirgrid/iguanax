
/*
MSH|^~\&|AcmeMed|Lab|Main HIS|St. Micheals|20110213144932||ADT^A03|9B38584D9903051F0D2B52CC0148965775D2D23FE4C51BE060B33B6ED27DA820|P|2.6|
EVN||20110213144532||||20110213145902|
PID|||4525285^^^ADT1||Smith^Tracy||19980210|F||Martian|86 Yonge St.^^ST. LOUIS^MO^51460|||||||10-346-6|284-517-569|
NK1|1|Smith^Gary|Second Cousin|
PV1||E||||||5101^Garland^Mary^F^^DR|||||||||||1318095^^^ADT1|||||||||||||||||||||||||20110213144956|
OBX|||WT^WEIGHT||102|pounds|
OBX|||HT^HEIGHT||32|cm|
*/

CodeMirror.defineMode('hl7', function(Config, Options){
   var DelimsRegex = Options.DelimiterRegex || /(\\|&|~|\^|\|)/;

   function token(Stream, State){
      if(Stream.string === Options.LfToken ||
         Stream.string === Options.CrToken ||
         Stream.string === (Options.CrToken + Options.LfToken))
      {
         Stream.next();
         return null;
      }

      var ch = Stream.peek();

      if(Stream.sol()){
         if(/\w/.test(ch)){
            Stream.eatWhile(/\w/);
            return "keyword";
         }
      }

      if(ch.match(DelimsRegex)){
         Stream.next();
         return "delim";
      }

//       if(/\d/.test(ch)){
//          Stream.eatWhile(/\d/);
//          return "number";
//       }

      Stream.next();
      return null;
   }

   return {
      token: token
   };
});
