#ifndef __DB_DATABASE_SQLITE_H__
#define __DB_DATABASE_SQLITE_H__
//---------------------------------------------------------------------------
//
...
// Description:
//
// This class handles calls to the SQLITE native C API. This class links the
...
class DBsqliteDll;
class DBlibImportSqlite;


// JULIAN just a skeleton for now

// JULIAN duplicated Sqlite class for now
//        probably has a lot of cruft to remove

class DBdatabaseSqlite : public DBdatabase
{
public:
   DBdatabaseSqlite(DBdatabaseType Type);
   ~DBdatabaseSqlite();

   static bool loadConfig(COLostream& LogStream, const COLstring& FileName);

   // initialize() attempts to load the Sqlite shared library.  If a Sqlite library has
   ...
   // reinitialize() will attempt to load a new Sqlite shared library, replacing the one
   // loaded by initialize() if applicable.  It must be used to load a new Sqlite shared
   ...
   //
   // reinitialize() cannot be used to unload all Sqlite shared libraries, and will always
   // leave DBdatabaseSqlite in a sane state, even if an exception is thrown.  Also,
   // it is safe to reinitialize() to the same library that is already loaded.
   //
   // *** We no longer support loading multiple Sqlite libraries (#10153).  reinitialize()
   // *** will fail unless no Sqlite library is loaded.  If readyToReinitialize() is false,
   ...
   // IMPLEMENTATION ONLY
   // This is used to initialize Sqlite's thread local storage
...
private:
   ...
   //#5699 Making DB_TEXT on Sqlite -> LONGTEXT
   const COLstring& dbTextType() const; //override DBdatabase

   DBdatabaseType m_Type; //Needed to tell whether MySQL or Sqlite. 

//   DBdatabaseSqlitePrivate* pMember; // JULIAN
   DBlibImportSqlite* pMember;
   DBsqliteDll&       SqliteDll;
   // JULIAN-CHECK m_BinaryOkay definitely not required as BLOB is supported in all DB now
   bool               m_BinaryOkay;


   DBdatabaseSqlite( DBdatabaseSqlite& Orig ); // not allowed
   DBdatabaseSqlite& operator=( DBdatabaseSqlite& Orig ); // not allowed
};
