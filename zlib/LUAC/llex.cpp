...
void luaX_init (lua_State *L) {
  COL_FUNCTION("luaX_init");
  int i;
  for (i=0; i<NUM_RESERVED; i++) {
    COL_VAR(luaX_tokens[i]);
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '=': {
        next(ls);
        if (ls->current != '=') return '=';
        else { next(ls); COL_TRC("== token"); return TK_EQ;}
      }
      case '<': {
        next(ls);
      ...
      case '>': {
        next(ls);
        if (ls->current != '=') { COL_TRC("> token"); return '>'; }
        else { next(ls); COL_TRC(">= token"); return TK_GE; }
      }
      case '~': {
        next(ls);
        if (ls->current != '=') { COL_TRC("~ token"); return '~'; }
      ...
      case '.': {
        save_and_next(ls);
        if (check_next(ls, ".")) {
           if (check_next(ls, ".")) { COL_TRC("... token"); return TK_DOTS;}   /* ... */
           else { COL_TRC(".. token"); return TK_CONCAT; }   /* .. */
        }
      ...
      default: {
        if (isspace(ls->current)) {
          ...
          {
            COL_TRC("Reserved word token '" << luaX_tokens[ts->tsv.reserved-1] << '\'');
